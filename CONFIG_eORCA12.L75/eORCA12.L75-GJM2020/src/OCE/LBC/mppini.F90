MODULE mppini
   ...
   !! $Id: mppini.F90 11640 2019-10-02 12:06:19Z clem $ 
...
!!$      IF(.NOT.Agrif_Root())   jpkglo = Agrif_Parent( jpkglo )
   ...
   SUBROUTINE mpp_init_readbot_strip( kjstr, kjcnt, ldoce )
      ...
      REAL(wp), DIMENSION(jpiglo,kjcnt) ::   zmsk, zbdy 
      !!----------------------------------------------------------------------
      !
      inumsave = numout   ;   numout = numnul   !   redirect all print to /dev/null
      !
      IF( numbot /= -1 ) THEN
         CALL iom_get( numbot, jpdom_unknown, 'mppmask', zmsk, kstart = (/1,kjstr/), kcount = (/jpiglo, kjcnt/) )
      ELSE
         zmsk(:,:) = 1.                         ! put a non-null value
      ENDIF
