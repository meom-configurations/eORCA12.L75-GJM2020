...
#  include "vectopt_loop_substitute.h90"
   ...
   !! $Id: zdftke.F90 11536 2019-09-11 13:54:18Z smasson $
...
902   IF( ios >  0 ) CALL ctl_nam ( ios , 'namzdf_tke in configuration namelist' )
      ...
      IF( nn_htau < 0   .OR.  nn_htau > 2 .AND. nn_htau /= 4 )   CALL ctl_stop( 'bad flag: nn_htau is 0, 1 ,2  or 4' )
#else
      IF( nn_htau < 0   .OR.  nn_htau > 1 )   CALL ctl_stop( 'bad flag: nn_htau is 0, 1 ' )
#endif
...
#if defined key_drakkar
         CASE( 2 )
!CT { TKE injection reduction
            htau(:,:) = MAX(  0.5_wp, MIN( 10._wp, 45._wp* ABS( SIN( rpi/180._wp * gphit(:,:) ) ) )   )
            IF(lwp) WRITE(numout,*) '       In case nn_etau = 1, # reduce the injection of surface TKE below the ML'
            IF(lwp) WRITE(numout,*) '                              en(ji,jj,jk) = en(ji,jj,jk) + rn_efr * en(ji,jj,1) * EXP( -fsdepw(ji,jj,jk) /'
            IF(lwp) WRITE(numout,*) '                              htau(ji,jj) ) * ( MAX(0.,1._wp - 4.*fr_i(ji,jj) ) ) * wmask(ji,jj,jk) * tmask(ji,jj,1)'
            IF(lwp) WRITE(numout,*) '                            # Reduce the depth penetration of TKE to 10m instead 30m beyong 45 degrees'
!CT }
